...
$DOCKER_CMD run $RUN_OPT --name golang-alpine-build  -v $(pwd):/work --workdir /work --entrypoint go --env-file ~/.gobuild-linux-cgo stevekieu/golang-alpine-build:latest build --tags "json1 secure_delete" --ldflags '-extldflags "-static" -w -s' -o webnote-linux-amd64-static main.go
