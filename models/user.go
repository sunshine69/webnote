...
func UserNew(in map[string]interface{}) *User {
	...
	n.SaltLength = u.MapLookup(in, "SaltLength", 12).(int8)
	...
	n.LastAttempt = u.MapLookup(in, "LastAttempt", 0).(int64)
	n.AttemptCount = u.MapLookup(in, "AttemptCount", 0).(int8)
	n.LastLogin = u.MapLookup(in, "LastLogin", 0).(int64)
	n.PrefID = u.MapLookup(in, "PrefID", 0).(int8)
